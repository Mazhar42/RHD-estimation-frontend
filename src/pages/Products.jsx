...
import { importItems, listDivisions } from "../api/items";
...
export default function Products() {
  ...
  const orgRegions = React.useMemo(
    () => orgRegionObjs.map((r) => r.name),
    [orgRegionObjs],
  );
  ...
  const [isManageOrganizationsOpen, setIsManageOrganizationsOpen] =
    useState(false);
  ...
  const makeInitialRegionRates = React.useCallback(
    (regions) => Object.fromEntries((regions || []).map((r) => [r, ""])),
    [],
  );
  ...
  const [isConfirmDeleteDivisionOpen, setIsConfirmDeleteDivisionOpen] =
    useState(false);
  ...
  const [totalSpecialItemCount, setTotalSpecialItemCount] = useState(0);
  ...
  useEffect(() => {
    // Load divisions and organizations in parallel
    (async () => {
      try {
        // Fetch divisions and organizations in parallel
        const [divs, orgs] = await Promise.all([
          listDivisions().catch(() => []),
          listOrganizations(),
        ]);
        ...
        const defaultOrg =
          orgs.find((o) => o.name === "RHD") || orgs[0] || null;
        setSelectedOrg(defaultOrg);

        if (defaultOrg) {
          // Fetch regions, items, and special items for default org IN PARALLEL
          const [regs] = await Promise.all([
            listRegions(defaultOrg.org_id),
            fetchItems(1, null, defaultOrg.name),
            fetchSpecialItems(1, null, defaultOrg.name),
          ]);
          ...
          setForm((f) => ({
            ...
            regionRates: makeInitialRegionRates(regs.map((r) => r.name)),
          }));
        } else {
          // No default org, just fetch items
          await Promise.all([
            fetchItems(1, null, ""),
            fetchSpecialItems(1, null, ""),
          ]);
        }
      } catch (e) {
  ...
  const changeOrganization = async (orgId) => {
    const org =
      organizations.find((o) => String(o.org_id) === String(orgId)) || null;
    setSelectedOrg(org);
    setCurrentPage(1); // Reset to page 1 when changing organization
    if (!org) {
      setOrgRegionObjs([]);
    ...
    try {
      // Fetch regions, items, and special items IN PARALLEL (much faster!)
      // Pass org.name directly so we don't rely on state updates
      const [regs] = await Promise.all([
        listRegions(org.org_id),
        fetchItems(1, null, org.name), // Pass org.name as override
        fetchSpecialItems(1, null, org.name), // Pass org.name as override
      ]);
  ...
  useEffect(() => {
    const units = [
      ...new Set(
        [...items, ...specialItems].map((item) => item.unit).filter(Boolean),
      ),
    ];
  ...
  useEffect(() => {
    const regions = orgRegions.length
      ? orgRegions
      : [
          ...new Set(
            [...items, ...specialItems]
              .map((item) =>
                item.region === "Cumilla Zone" ? "Comilla Zone" : item.region,
              )
              .filter(Boolean),
          ),
        ];
  ...
  // Fetch items when page changes or page size changes
  useEffect(() => {
    if (!selectedOrg) return;
    fetchItems(currentPage);
  }, [currentPage, selectedOrg, itemsPerPage]);

  // Debounced search: fetch items when search parameters change
  useEffect(() => {
    if (!selectedOrg) return;
  ...
  // Fetch special items when special page changes or page size changes
  useEffect(() => {
    if (!selectedOrg) return;
    fetchSpecialItems(specialCurrentPage);
  }, [specialCurrentPage, selectedOrg, specialItemsPerPage]);

  // Debounced search for special items
  useEffect(() => {
    if (!selectedOrg) return;
    ...
    searchDebounceTimer.current = setTimeout(() => {
      ...
      fetchSpecialItems(1, specialSearch);
    }, 500);
  ...
  const fetchItems = async (
    ...
    try {
      ...
      // Use passed org name or fall back to selectedOrg
      const orgName = orgNameOverride || selectedOrg?.name;
      ...
      if (filters.rate) {
        ...
        if (!isNaN(rate)) {
          switch (filters.rateOperator) {
            case "==":
              params.search = `rate:${rate}`;
              break; // Simplified - may need backend support
            case ">":
              params.rate_min = rate;
              break;
            case "<":
              params.rate_max = rate;
              break;
            default:
              break;
          }
      ...
      const countRes = await apiClient.get(`/items/count`, {
        params: countParams,
      });
      setTotalItemCount(countRes.data?.count || 0);
    } catch (e) {
      const msg = e?.response?.data?.detail || "Failed to load items";
      setItemsError(msg);
    } finally {
  ...
  const fetchSpecialItems = async (
    ...
    try {
      ...
      const filters = searchFilters || specialSearch;
      const skip = (pageNum - 1) * specialItemsPerPage;

      const params = {
        skip,
        limit: specialItemsPerPage,
      };
      ...
      const countRes = await apiClient.get(`/items/count`, {
        params: countParams,
      }); // Note: may need /items/special/count
      setTotalSpecialItemCount(countRes.data?.count || 0);
    } catch (e) {
  ...
  const handleRegionRateChange = (region, value) => {
    setForm((prev) => ({
      ...prev,
      regionRates: { ...prev.regionRates, [region]: value },
    }));
  };
  ...
  const addItem = async (e) => {
    ...
    const hasAtLeastOneRate = Object.values(form.regionRates).some(
      (v) => String(v).trim() !== "",
    );
    ...
    if (requiredMissing) {
      if (orgRegions.length === 0) {
        setAddItemError(
          `No regions for ${form.organization || selectedOrg?.name || "organization"}. Add a region first.`,
        );
      } else {
        setAddItemError(
          !hasAtLeastOneRate
            ? "Provide at least one region rate"
    ...
    const payloads = orgRegions
      ...
      .map((region) => ({
        ...base,
        rate: parseFloat(form.regionRates[region]),
        region,
      }));
    try {
    ...
    setForm({
      ...
      regionRates: makeInitialRegionRates(orgRegions),
    });
    setAddItemError("");
  ...
  const handleAddRegion = async () => {
    if (!newRegionName.trim()) {
      setAddRegionError("Input field is empty");
      return;
    }
    if (!selectedOrg?.org_id) {
      setAddRegionError("Select an organization first");
      return;
    }
    try {
  ...
  const handleAddOrganization = async () => {
    if (!newOrgName.trim()) {
      setAddOrgError("Input field is empty");
      return;
    }
    try {
      ...
      setOrganizations(orgs);
      const created = orgs.find(
        (o) => o.name.toLowerCase() === newOrgName.trim().toLowerCase(),
      );
  ...
  const updateItem = async () => {
    ...
    const payload = {
      ...editItem,
      rate: editItem.rate ? parseFloat(editItem.rate) : null,
      division_id: parseInt(editItem.division_id),
    };
  ...
  const submitImport = async () => {
    console.log(
      "[IMPORT] submitImport called, importFile:",
      importFile ? { name: importFile.name, size: importFile.size } : null,
      "mode:",
      importMode,
    );
    if (!importFile) {
    ...
    try {
      console.log("[IMPORT] Starting upload...");
      setIsImporting(true);
      setImportBanner({
        type: "info",
        message: "Importing item master… Please wait.",
      });
      console.log("[IMPORT] Calling importItems API...");
      const res = await importItems(importFile, importMode, (evt) => {
        console.log("[IMPORT] Upload progress:", evt.loaded, "/", evt.total);
        if (!evt.total) return;
      ...
      // Always reload items if any were processed
      if (processed > 0) {
        setCurrentPage(1);
        await fetchItems(1);
      }
      ...
      if (totalErrors > 0 && processed === 0) {
        // Total failure — no items imported
        const errorPreview = errors
          .slice(0, 5)
          .map((e) => "• " + e)
          .join("\n");
        const errorMsg = `Import failed: ${totalErrors} parsing errors.\n\n${errorPreview}${totalErrors > 5 ? "\n\n... and " + (totalErrors - 5) + " more errors" : ""}`;
        setImportError(errorMsg);
        setImportBanner({
          type: "error",
          message: `Failed to import: ${totalErrors} rows had errors`,
        });
      } else if (totalErrors > 0 && processed > 0) {
        // Partial success
        setImportBanner({
          type: "warning",
          message: `Imported ${processed} item(s) with ${totalErrors} error(s). Some rows were skipped.`,
        });
        setImportError("");
      } else {
        // Full success
        setImportBanner({
          ...
          message:
            processed > 0
              ? `Imported ${processed} item(s) successfully${skipped > 0 ? `, skipped ${skipped}` : ""}.`
      ...
      setImportFile(null);
      setUploadProgress(0);
      console.log("[IMPORT] Upload completed successfully");
    } catch (err) {
      ...
      console.log("[IMPORT] Setting isImporting to false");
  ...
  const handleImportClick = async () => {
    console.log(
      "[IMPORT] handleImportClick triggered, importFile:",
      importFile ? { name: importFile.name } : null,
    );
    if (!importFile) {
      console.log(
        "[IMPORT] No file selected, showing error and opening picker",
      );
    ...
    console.log("[IMPORT] File selected, calling submitImport()");
    await submitImport();
  };
      ...
      items.filter((item) => {
        ...
        return (
          (!search.division ||
            item.division?.name
              .toLowerCase()
              .includes(search.division.toLowerCase())) &&
          (!search.code ||
            item.item_code.toLowerCase().includes(search.code.toLowerCase())) &&
          (!search.description ||
  ...
  const isStringColumn = (col) =>
    ["division", "code", "description", "unit", "region"].includes(col);
  ...
  const isMassFilterActive =
    Array.isArray(massFilters) &&
    massFilters.some((f) => String(f.value).trim() !== "");
  const deleteCandidates = isMassFilterActive
    ? massFilteredItems
    : filteredItems;
  ...
  const bulkDeleteFiltered = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        ids.map((id) => apiClient.delete(`/items/${id}`)),
      );
      ...
      const msg =
        err?.response?.data?.detail || "Mass delete encountered an error.";
      setToast(msg);
      setTimeout(() => setToast(null), 3000);
    } finally {
  ...
  const bulkDeleteFilteredWithProgress = async () => {
    ...
    try {
      ...
      for (let i = 0; i < ids.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(
          batch.map((id) => apiClient.delete(`/items/${id}`)),
        );
        const succeededInBatch = results.filter(
      ...
      if (failed === 0) {
        ...
        setToast(
          `Successfully deleted ${completed} item(s). Failed ${failed}.`,
        );
      }
      setTimeout(() => setToast(null), 3000);
    } catch (err) {
  ...
  // Items are already filtered and paginated from the server,
  // so no need for client-side filtering or slicing
  const currentItems = groupedRows;
  const totalPages = Math.ceil(totalItemCount / itemsPerPage);
  const showingStart =
    totalItemCount > 0 && currentPage > 0
      ? (currentPage - 1) * itemsPerPage + 1
      : 0;
  const showingEnd = totalItemCount > 0
    ? Math.min(currentPage * itemsPerPage, totalItemCount)
    : 0;
  ...
  const [columnWidths, setColumnWidths] = useState(() => {
    try {
      ...
      return saved
        ? JSON.parse(saved)
        : { division: 150, code: 150, description: 300 };
    } catch {
  ...
  const [
    ...
    setIsDeleteSelectedSpecialConfirmOpen,
  ] = useState(false);
    ...
    const timer = setTimeout(() => {
      try {
        localStorage.setItem(
          ...
          JSON.stringify(specialColumnWidths),
        );
      } catch {}
    }, 500);
    return () => clearTimeout(timer);
  ...
  const [isDeleteSelectedConfirmOpen, setIsDeleteSelectedConfirmOpen] =
    useState(false);
  const [isConfirmDeleteGroupOpen, setIsConfirmDeleteGroupOpen] =
    useState(false);
  ...
  const currentPageItemIds = () =>
    currentItems.map((it) => it.item_id).filter(Boolean);
  ...
  const deleteSelectedItems = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.detail || "Delete selected encountered an error.";
      setToast(msg);
      setTimeout(() => setToast(null), 3000);
    }
  };
  ...
  const getItemsForGroup = (key) => {
    ...
    return items.filter(
      (it) =>
        String(it.division_id) === String(division_id) &&
        String(it.item_code) === String(item_code),
    );
  };
  ...
  const saveEditGroup = async () => {
    ...
    const groupItems = getItemsForGroup(
      `${base.division_id}|${base.item_code}`,
    );
    try {
      const ops = [];
        ...
        if (existing) {
          ops.push(
            apiClient.put(`/items/${existing.item_id}`, {
              ...base,
              rate: val !== "" && val != null ? parseFloat(val) : null,
      ...
      const msg =
        err?.response?.data?.detail || "Failed to update grouped item.";
      setToast(msg);
      setTimeout(() => setToast(null), 3000);
    }
  };
  ...
  const deleteSelectedGroup = async () => {
    ...
    try {
      const results = await Promise.allSettled(
        groupItems.map((it) => apiClient.delete(`/items/${it.item_id}`)),
      );
      const succeeded = results.filter((r) => r.status === "fulfilled").length;
      const failed = results.length - succeeded;
      setCurrentPage(1);
      ...
      const msg =
        err?.response?.data?.detail || "Failed to delete grouped item.";
      setToast(msg);
      setTimeout(() => setToast(null), 3000);
    }
  };
  ...
  const groupedFilteredSpecialRows = React.useMemo(
    () => groupedSpecialRows,
    [groupedSpecialRows],
  );
  ...
  const currentSpecialItems = groupedFilteredSpecialRows;
  const totalSpecialPages = Math.ceil(totalSpecialItemCount / specialItemsPerPage);
  const showingSpecialStart =
    totalSpecialItemCount > 0 && specialCurrentPage > 0
      ? (specialCurrentPage - 1) * specialItemsPerPage + 1
      : 0;
  const showingSpecialEnd = totalSpecialItemCount > 0
    ? Math.min(specialCurrentPage * specialItemsPerPage, totalSpecialItemCount)
    : 0;
  ...
  const deleteSelectedSpecialGroup = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.detail || "Failed to delete grouped special item.";
      setToast(msg);
      setTimeout(() => setToast(null), 3000);
    }
  };
    ...
    <div className="item-master-font relative bg-white p-4 sm:p-6 -mx-6 -mb-6 w-[calc(100%+3rem)]">
      ...
      {activeTab === "special" && (
        <div className="mb-6 bg-white rounded-lg">
          ...
          {specialItemsError && (
            <div className="text-sm text-red-600">{specialItemsError}</div>
          )}
            ...
            <>
                  ...
                  >
                    <table className="min-w-full border-collapse table-fixed">
                      <thead className="sticky top-0 z-10 bg-gray-100 border-b-2 border-gray-200">
                        <tr>
                          {isAdmin && (
                            <th className="px-2 py-1 text-center text-xs font-bold border-r min-w-[40px] text-gray-800 border-gray-200">
                              Sel
                            </th>
                          )}
                          ...
                          <th
                            style={{
                              width: specialColumnWidths.division,
                              minWidth: specialColumnWidths.division,
                            }}
                            className="relative px-2 py-1 text-left text-xs font-bold border-r text-gray-800 border-gray-200 group select-none"
                          >
                            Division
                            <div
                              className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize bg-transparent group-hover:bg-teal-400 z-20"
                              onMouseDown={(e) =>
                                startResizingSpecial(e, "division")
                              }
                            />
                          </th>
                          ...
                          <th
                            style={{
                              width: specialColumnWidths.code,
                              minWidth: specialColumnWidths.code,
                            }}
                            className="relative px-2 py-1 text-left text-xs font-bold border-r text-gray-800 border-gray-200 group select-none"
                          >
                            Code
                          ...
                          <th
                            style={{
                              width: specialColumnWidths.description,
                              minWidth: specialColumnWidths.description,
                            }}
                            className="relative px-2 py-1 text-left text-xs font-bold border-r text-gray-800 border-gray-200 group select-none"
                          >
                            Description
                            <div
                              className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize bg-transparent group-hover:bg-teal-400 z-20"
                              onMouseDown={(e) =>
                                startResizingSpecial(e, "description")
                              }
                            />
                          </th>

                          <th className="px-2 py-1 text-left text-xs font-bold border-r min-w-[120px] sm:min-w-[150px] text-gray-800 border-gray-200">
                            Unit
                          </th>
                          <th className="px-2 py-1 text-left text-xs font-bold border-r min-w-[140px] text-gray-800 border-gray-200">
                            Organization
                          </th>
                          {orgRegions.map((r) => (
                            <th
                              key={r}
                              className="px-2 py-1 text-right text-xs font-bold border-r min-w-[140px] text-gray-800 border-gray-200"
                            >
                              {r}
                            </th>
                          ))}
                        ...
                        <tr>
                          ...
                          <th className="px-2 py-1 border-r border-gray-200">
                            <input
                              type="text"
                              placeholder="Search..."
                              value={specialSearch.description}
                              onChange={(e) =>
                                setSpecialSearch({
                                  ...specialSearch,
                          ...
                          {orgRegions.map((r) => (
                            <th
                              key={r}
                              className="px-2 py-1 border-r min-w-[140px] border-gray-200"
                            ></th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {groupedSpecialRows.length === 0 ? (
                          <tr>
                            <td
                              colSpan={
                                5 + orgRegions.length + (isAdmin ? 1 : 0)
                              }
                              className="p-4 sm:p-8 text-center text-gray-600"
                            >
                          ...
                          currentSpecialItems.map((row, i) => (
                            <tr
                              key={`${row.item_code}|${row.division_id ?? row.division?.name ?? i}`}
                            ...
                            >
                                ...
                                <td className="px-2 py-1 text-center border-r border-gray-200">
                                  <input
                                    ...
                                    checked={
                                      ...
                                      `${row.division_id}|${row.item_code}`
                                    }
                                    onChange={() =>
                                      setSelectedSpecialGroupKey((prev) =>
                                        prev ===
                                        `${row.division_id}|${row.item_code}`
                                          ? null
                                          : `${row.division_id}|${row.item_code}`,
                              ...
                              <td className="px-2 py-1 whitespace-normal break-words text-xs border-r text-gray-800 border-gray-200">
                                {row.division?.name || "—"}
                              </td>
                              <td className="px-2 py-1 whitespace-normal break-words text-xs border-r text-gray-800 border-gray-200">
                                {row.item_code}
                              ...
                              <td className="px-2 py-1 whitespace-nowrap text-xs border-r text-gray-800 border-gray-200">
                                {row.organization || "RHD"}
                              </td>
                              {orgRegions.map((r) => (
                                <td
                                  key={r}
                                  className="px-2 py-1 whitespace-nowrap text-xs border-r text-gray-800 border-gray-200 text-right"
                                >
                                  {row.rates[r] != null && row.rates[r] !== ""
                      ...
                      <div className="sticky bottom-0 z-20 bg-gray-800 text-white px-3 py-2 border-t border-gray-700 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            onClick={() => beginEditSelectedSpecialGroup()}
                          >
                            Edit
                          </button>
                          <button
                            ...
                            onClick={() =>
                              setIsDeleteSelectedSpecialConfirmOpen(true)
                            }
                          >
                            Delete
                          </button>
                          <button
                            ...
                            onClick={() => setSelectedSpecialGroupKey(null)}
                          >
                            Clear selection
                          </button>
                        </div>
              ...
              <div className="flex flex-wrap justify-between items-center p-4 gap-3">
                <div className="text-xs text-gray-600">
                  Showing{" "}
                  {new Intl.NumberFormat("en-US").format(showingSpecialStart)}–
                  {new Intl.NumberFormat("en-US").format(showingSpecialEnd)} of{" "}
                  {new Intl.NumberFormat("en-US").format(totalSpecialItemCount)}{" "}
                  rows
                </div>
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() =>
                        setSpecialCurrentPage((p) => Math.max(p - 1, 1))
                      }
                    ...
                    <button
                      onClick={() =>
                        setSpecialCurrentPage((p) =>
                          Math.min(p + 1, totalSpecialPages || 1),
                        )
                      }
                      disabled={
                        specialCurrentPage === totalSpecialPages ||
                        totalSpecialPages === 0
                      }
                      className="px-2 py-1 border rounded text-xs disabled:opacity-50 hover:bg-gray-100 text-gray-600"
                    >
      ...
      {activeTab === "items" && (
        <>
              ...
              {isAdmin && (
                <>
                  <button
                    ...
                    className="bg-teal-700 hover:bg-teal-900 text-white text-xs font-extralight h-6 px-4 rounded inline-flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-extralight h-6 px-4 rounded inline-flex items-center gap-1"
                  >
                    <FaPlus className="w-3 h-3" />
                    <span>Add Item</span>
                  </button>
                  ...
                  <button
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        setIsManageOrganizationsOpen(true);
                      }
                    }}
          ...
          {importBanner && (
            <div
              className={`mt-3 p-3 rounded-md border text-sm ${importBanner.type === "error" ? "bg-red-50 text-red-700 border-red-300" : importBanner.type === "warning" ? "bg-orange-50 text-orange-700 border-orange-300" : importBanner.type === "info" ? "bg-indigo-50 text-indigo-700 border-indigo-300" : "bg-emerald-50 text-emerald-700 border-emerald-300"}`}
            >
            ...
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-xl z-50 relative border border-gray-200">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Create a division to group related items.
                </p>
                <div className="mt-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (addDivisionError && e.target.value.trim())
                        setAddDivisionError("");
                    }}
            ...
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-2xl z-50 relative border border-gray-200">
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Fill in item details including division, unit, and
                  multi-region rates.
                </p>
                {addItemError && (
                  ...
                  <div className="relative">
                    ...
                    >
                      <span>
                        {divisions.find(
                          (d) =>
                            String(d.division_id) === String(form.division_id),
                        ...
                        {divisionsForOrg.map((d) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={() =>
                                requestDeleteDivision(d.division_id)
                              }
                            >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                    {orgRegions.map((r) => (
                      <div key={r} className="flex items-center gap-2">
                        <label className="text-[11px] text-gray-700 whitespace-nowrap">
                          {r}
                        </label>
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            handleRegionRateChange(r, e.target.value)
                          }
                          placeholder="Rate"
                    ...
                    {orgRegions.length === 0 && (
                      <div className="text-xs text-gray-600">
                        No regions defined for{" "}
                        {form.organization ||
                          selectedOrg?.name ||
                          "organization"}
                        . Use Manage Regions to add.
                      </div>
                    )}
                  </div>
                  ...
                  <div className="fixed inset-0 bg-black/20 flex justify-center items-center z-[60]">
                    <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-sm p-4">
                      ...
                      <p className="text-xs text-gray-600 mb-4">
                        Do you want to delete this division? This will remove
                        related items.
                      </p>
          ...
          {isManageRegionsOpen && (
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md z-50 relative border border-gray-200">
                <button
                  onClick={() => setIsManageRegionsOpen(false)}
                  className="absolute top-3 right-3 inline-flex items-center justify-center w-9 h-9 rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900 transition"
                >
                  <svg
                    className="w-5 h-5"
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Add zones for the selected organization.
                </p>
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      className="border border-gray-300 p-2 rounded w-full text-xs bg-gray-100"
                      readOnly
                  ...
                  <div className="max-h-40 overflow-auto border border-gray-200 rounded p-2">
                    {orgRegionObjs.length === 0 ? (
                      ...
                      orgRegionObjs.map((r) => (
                        <div
                          key={r.region_id}
                          className="group flex items-center justify-between text-xs py-2 px-2 rounded hover:bg-gray-50 border-b border-gray-100"
                        >
                          ...
                          <div className="flex items-center gap-2">
                            {editingRegionId === r.region_id ? (
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      setForm((f) => ({
                                        ...f,
                                        regionRates: makeInitialRegionRates(
                                          regs.map((rr) => rr.name),
                                        ),
                                      }));
                              ...
                              <>
                                <button
                                  onClick={() => {
                                    setEditingRegionId(r.region_id);
                                ...
                                <button
                                  onClick={() =>
                                    setConfirmDeleteRegionId(r.region_id)
                                  }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (addRegionError && e.target.value.trim())
                          setAddRegionError("");
                      }}
                    />
                    {addRegionError && (
                ...
                <div className="mt-4 flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className="bg-teal-700 hover:bg-teal-900 text-white font-medium py-1 px-3 rounded text-xs"
                  >
                    Add Region
                  </button>
                </div>
          ...
          {isManageOrganizationsOpen && (
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md z-50 relative border border-gray-200">
                <button
                  onClick={() => setIsManageOrganizationsOpen(false)}
                  className="absolute top-3 right-3 inline-flex items-center justify-center w-9 h-9 rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900 transition"
                >
                  <svg
                    className="w-5 h-5"
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Add or remove organizations and pick the active one.
                </p>
                <div className="space-y-3">
                  <div className="max-h-40 overflow-auto border border-gray-200 rounded p-2">
                        ...
                        >
                          {editingOrgId === o.org_id ? (
                            ...
                            <button
                              ...
                              onClick={() => changeOrganization(o.org_id)}
                            >
                              {o.name}
                            </button>
                          )}
                          <div className="flex items-center gap-2">
                            {editingOrgId === o.org_id ? (
                              ...
                              <>
                                ...
                                <button
                                  onClick={() =>
                                    setConfirmDeleteOrgId(o.org_id)
                                  }
                                  className="text-red-600 hover:text-red-800 inline-flex items-center gap-1"
                                >
                                  <FaTrash className="w-3 h-3" />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (addOrgError && e.target.value.trim())
                          setAddOrgError("");
                      }}
                    />
                    {addOrgError && (
                      <p className="mt-1 text-xs text-red-600">{addOrgError}</p>
                    )}
                  </div>
                </div>
                <div className="mt-4 flex justify-end gap-2">
          ...
          {isImportModalOpen && (
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-xl z-50 relative border border-gray-200">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Upload a CSV or XLSX in the Item Master format. Choose whether
                  to replace all items or add on top.
                </p>
          ...
          {isMassDeleteModalOpen && (
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div
                ref={massDeleteContainerRef}
                className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-xl z-50 relative border border-gray-200"
              >
                <button
                ...
                <p className="text-sm text-gray-700 mb-3">
                  Use the filter below to target items. This action cannot be
                  undone.
                </p>
                <div className="mb-4">
                      ...
                      <div className="mt-2">
                        ...
                        <div className="w-full h-2 bg-red-100 rounded">
                          <div
                            ...
                            style={{
                              width: `${massDeleteTotal ? Math.round((massDeleteCompleted / massDeleteTotal) * 100) : 0}%`,
                            }}
                          />
                        </div>
                      </div>
                ...
                <p className="mt-3 text-xs text-gray-500">
                  Note: Items referenced elsewhere may fail to delete.
                  Successful deletions will proceed.
                </p>
              </div>
            </div>
          )}
                ...
                <table className="min-w-full border-collapse table-fixed">
                  <thead className="sticky top-0 z-10 bg-gray-100 border-b-2 border-gray-200">
                    <tr>
                      ...
                      <th
                        style={{
                          width: columnWidths.division,
                          minWidth: columnWidths.division,
                        }}
                        className="relative px-2 py-1 text-left text-xs font-bold border-r text-gray-800 border-gray-200 group select-none"
                      >
                        Division
                    ...
                    <tr>
                      ...
                      <th className="px-2 py-1 border-r min-w-[120px] sm:min-w-[150px] border-gray-200">
                        <select
                          value={search.unit}
                          onChange={(e) =>
                            setSearch({ ...search, unit: e.target.value })
                          }
                          className="w-full text-xs px-2 py-1 border rounded-md transition-colors bg-white text-gray-900 border-gray-300 placeholder-gray-500 focus:border-teal-500 focus:ring-1 focus:ring-teal-200"
                        >
                    ...
                    {groupedRows.length === 0 ? (
                      ...
                      currentItems.map((row, i) => (
                        ...
                        >
                          ...
                          <td
                            className="px-2 py-1 whitespace-normal break-words text-xs border-r text-gray-800 border-gray-200"
          ...
          <div
            ...
            className="flex flex-wrap justify-between items-center p-4 gap-3"
          >
            <div className="text-xs text-gray-600">
              Showing {new Intl.NumberFormat("en-US").format(showingStart)}–
              {new Intl.NumberFormat("en-US").format(showingEnd)} of{" "}
              {new Intl.NumberFormat("en-US").format(totalItemCount)} rows
            </div>
            <div className="flex items-center gap-2 text-gray-600">
              <select
                value={itemsPerPage}
                onChange={(e) => {
                  setItemsPerPage(parseInt(e.target.value, 10) || 50);
                  setCurrentPage(1);
                }}
                className="text-xs border rounded px-2 py-1 text-gray-700"
              >
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      if (!Number.isNaN(val)) {
                        setCurrentPage(
                          Math.min(Math.max(val, 1), totalPages || 1),
                        );
          ...
          {isItemDetailOpen && detailItem && (
            <div
              ...
              onClick={(e) => {
                if (e.target === e.currentTarget) setIsItemDetailOpen(false);
              }}
            >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <span
                      ...
                      title={detailItem.item_description}
                    >
                      {detailItem.item_description}
                    </span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-700">Unit:</span>{" "}
                    <span className="text-gray-900">{detailItem.unit}</span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-700">Rate:</span>{" "}
                    <span className="text-gray-900">
                      {formatRate(detailItem.rate)}
                    </span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-700">Region:</span>{" "}
                    <span className="text-gray-900">{detailItem.region}</span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-700">
                      Item ID:
                    </span>{" "}
                    <span className="text-gray-900">{detailItem.item_id}</span>
                  </div>
                </div>
                <div className="mt-4 flex justify-end gap-2">
                  <button
            ...
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-2xl z-50 relative border border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <select
                    name="division_id"
                    value={editItem.division_id}
                    onChange={(e) =>
                      setEditItem({ ...editItem, division_id: e.target.value })
                    }
                  ...
                  <input
                    ...
                    className="border border-gray-300 p-3 rounded-lg col-span-2 focus:outline-none focus:ring-2 focus:ring-teal-500"
                  />
                  <select
                    name="unit"
                    value={editItem.unit}
            ...
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm flex justify-center items-center z-50">
              <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-3xl z-50 relative border border-gray-200">
                ...
                <h2 className="text-xl sm:text-2xl font-semibold mb-3 text-gray-900">
                  Edit Grouped Item Rates
                </h2>
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <select
                    name="division_id"
                    value={editGroupForm.division_id}
                    onChange={(e) =>
                      setEditGroupForm({
                        ...editGroupForm,
                  ...
                  <input
                    name="item_description"
                    value={editGroupForm.item_description}
                    onChange={(e) =>
                      setEditGroupForm({
                        ...editGroupForm,
                        item_description: e.target.value,
                  ...
                  <input
                    name="organization"
                    value={editGroupForm.organization}
                    onChange={(e) =>
                      setEditGroupForm({
                        ...editGroupForm,
          ...
          {isConfirmDeleteGroupOpen && (
            <div className="fixed inset-0 bg-black/20 flex justify-center items-center z-[60]">
              <div className="bg-white rounded-xl shadow-xl border border-gray-200 w/full max-w-sm p-4">
                ...
                <p className="text-xs text-gray-600 mb-4">
                  Delete all region rates for this item code in the selected
                  division? This may remove multiple rows and cannot be undone.
                </p>
                <div className="flex justify-end gap-2">
                  <button
      ...
      {isDeleteSelectedSpecialConfirmOpen && (
        <div className="fixed inset-0 bg-black/20 flex justify-center items-center z-[60]">
          <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-sm p-4">
            <h3 className="text-sm font-semibold text-gray-900 mb-2">
              Delete Selected Special Items
            </h3>
            <p className="text-xs text-gray-600 mb-4">
              Are you sure you want to delete this grouped item? This action
              cannot be undone.
            </p>
            <div className="flex justify-end gap-2">
              <button
