  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      ...
      <form onSubmit={createEstimation} className="flex gap-2 mb-4">
        ...
        <button className="bg-green-600 text-white px-4 rounded">Create</button>
      </form>
      ...
      <div className="space-y-2">
        {estimations.map(es => (
          <div key={es.estimation_id} className="p-3 border rounded flex justify-between items-center">
            ...
            <div>
              <Link to={`/estimations/${es.estimation_id}`} className="bg-blue-600 text-white px-3 py-1 rounded">Open</Link>
            </div>
          </div>
        ))}
      </div>
